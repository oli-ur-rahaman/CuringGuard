@echo off
setlocal
title CuringGuard: Open on PC and Mobile
...
echo [2/7] Ensuring secure mobile tunnel tool exists...
...
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
  "$deadline=(Get-Date).AddSeconds(90); $url=''; while((Get-Date) -lt $deadline){ if(Test-Path 'mobile-https-tunnel.log'){ $match=Select-String -Path 'mobile-https-tunnel.log' -Pattern 'https://[-0-9a-z]+\.trycloudflare\.com' | Select-Object -Last 1; if($match){ $m=[regex]::Match($match.Line,'https://[-0-9a-z]+\.trycloudflare\.com'); if($m.Success){ $url=$m.Value; break } } }; Start-Sleep -Seconds 1 }; if(-not $url){ exit 1 }; Set-Content -Path 'mobile-https-url.txt' -Value $url; Set-Clipboard -Value $url; Write-Host ''; Write-Host '===================================================' -ForegroundColor Green; Write-Host ' MOBILE HTTPS URL ' -ForegroundColor Green; Write-Host '===================================================' -ForegroundColor Green; Write-Host $url -ForegroundColor Cyan; Write-Host '===================================================' -ForegroundColor Green"
if errorlevel 1 (
