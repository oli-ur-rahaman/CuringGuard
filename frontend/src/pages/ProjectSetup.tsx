...
import { 
  ...
  ChevronRight, Plus, UserPlus, HardHat, MoreVertical, Map, ChevronDown
} from 'lucide-react';
...
export default function ProjectSetup() {
  ...
  const projects = [
    { id: 1, name: 'Dubai Creek Harbor Tower', status: 'Active' },
    { id: 2, name: 'Riyadh Metro Extension', status: 'Planning' },
  ];

  const packages = [
    { id: 1, projectId: 1, name: 'Phase 1: Substructure' },
    { id: 2, projectId: 1, name: 'Phase 2: Superstructure' },
    { id: 3, projectId: 1, name: 'Phase 3: Podium Integration' },
  ];

  const structures = [
    { id: 1, packageId: 1, name: 'Main Raft Foundation', assignedTo: 'Oli Contractors LLC' },
    { id: 2, packageId: 1, name: 'Basement Parking Walls', assignedTo: null },
  ];
      ...
      <div className="flex-1 min-h-0 flex flex-col lg:flex-row gap-4 md:gap-6 overflow-y-auto lg:overflow-hidden pb-10 lg:pb-0">
         ...
         <div className="flex flex-col flex-1 min-h-[300px] bg-white border border-slate-200 rounded-3xl overflow-hidden shadow-sm shadow-slate-200/50">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-2 bg-slate-50/50 no-scrollbar">
               {projects.map(p => (
                 <div key={p.id} onClick={() => {setActiveProject(p.id); setActivePackage(0);}} 
                    ...
                    <div>
                       ...
                       <span className={`text-[10px] font-extrabold uppercase tracking-widest px-2.5 py-1 rounded-full mt-2 inline-block shadow-sm border ${p.status === 'Active' ? 'bg-green-100 text-green-800 border-green-200' : 'bg-slate-100 text-slate-600 border-slate-200'}`}>{p.status}</span>
                    </div>
         ...
         <div className="flex flex-col flex-1 min-h-[300px] bg-white border border-slate-200 rounded-3xl overflow-hidden shadow-sm shadow-slate-200/50">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-2 bg-slate-50/50 no-scrollbar">
               {packages.filter(pkg => pkg.projectId === activeProject).map(pkg => (
               ...
               {packages.filter(pkg => pkg.projectId === activeProject).length === 0 && (
         ...
         <div className="flex flex-col flex-1 min-h-[350px] bg-white border border-slate-200 rounded-3xl overflow-hidden shadow-sm shadow-slate-200/50">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-slate-50/50 no-scrollbar">
               {activePackage !== 0 ? structures.filter(s => s.packageId === activePackage).map(s => (
                 <div key={s.id} className="p-5 rounded-2xl border-2 border-slate-200 bg-white hover:border-purple-300 transition-all shadow-sm group">
                    ...
                    <div className="bg-slate-100/50 rounded-xl p-4 border border-slate-200">
                       <div className="text-[11px] font-extrabold text-slate-500 uppercase tracking-widest mb-3 flex items-center justify-between">
                         Assigned Contractor
                         {s.assignedTo && <span className="bg-green-200/80 text-green-900 px-2.5 py-0.5 rounded-full text-[9px] shadow-[0_1px_2px_rgba(0,0,0,0.1)]">Operating</span>}
                       </div>
                       ...
                       {s.assignedTo ? (
                         <div className="flex items-center justify-between">
                            <div className="flex items-center gap-3">
                               ...
                               <span className="font-extrabold text-slate-900 text-sm xl:text-base">{s.assignedTo}</span>
                            </div>
                         ...
                         <div className="relative">
                           <select className="appearance-none w-full bg-white border-2 border-dashed border-slate-300 rounded-xl py-3 pl-10 pr-10 hover:border-purple-400 focus:outline-none focus:border-purple-500 focus:ring-4 focus:ring-purple-500/10 transition-all text-slate-600 font-extrabold text-[11px] lg:text-xs tracking-wide cursor-pointer shadow-sm">
                              <option value="" disabled selected>DEPLOY CONTRACTOR...</option>
                              <option value="Oli Contractors LLC">Oli Contractors LLC</option>
                              <option value="Alpha Structural Builders">Alpha Structural Builders</option>
                              <option value="Desert Foundation Group">Desert Foundation Group</option>
                           </select>
